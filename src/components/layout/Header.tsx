        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex lg:hidden items-center gap-2">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-xs font-extrabold transition-all border active:scale-95 cursor-pointer shadow-sm ${
                isScrolled 
                  ? 'border-emerald-200 bg-emerald-50 text-emerald-900' 
                  : 'border-white/30 bg-black/40 text-white backdrop-blur-md hover:bg-black/50'
              }`}
              title="Switch currency USD / IDR"
              aria-label="Switch display currency"
            >
              <Globe className="w-3.5 h-3.5 text-amber-400 shrink-0" />
              <span>{currency === 'USD' ? 'USD ($)' : 'IDR (Rp)'}</span>
            </button>
